...
export async function setDriveEvents(drive, driveName = '') {
    const debouncedUpdateNotify = debounce(() => emitter.broadcast('notify-info', `${driveName} drive updated`), 1000);
...
export default class extends hyperdrive {
    ...
    async $readAllDirFiles(drive_src, dest = '/') {
        return await this.check(async () => {
            let files = [];
            let dirList;
            if (typeof drive_src === 'string')
                dirList = await this.$readDir(drive_src, { dest });
            else
                dirList = drive_src;
            for (let item of dirList) {
                ...
                if (isdir) {
                    ...
                    if (items.length)
                        files = [...files, ...(await this.$readAllDirFiles(item.path, dest))];
                }
                else {
                    files = [...files, _dest];
                }
        ...
        await this.check(async () => {
            for (let item of dirList) {
                ...
                if (isdir) {
                    ...
                    emitter.log(items);
                    if (items.length)
                        await this.$fsWriteDir(items, fs_dest, { drive });
                }
                else {
                    ...
                    if (drive)
                        ...
                        fs.createReadStream(join(config.fs, item.path)).pipe(destFile);
                }
            }
        });
    }
            ...
            for (let item of dirList) {
                ...
                if (isdir) {
                    ...
                    if (items === null || items === void 0 ? void 0 : items.length)
                        await this.$driveWriteDir(items, dest, { drive, isdrive });
                }
                else {
    ...
    // GRUD
    async $list(dir = '/', recursive = false, { offset = 0, limit = 50, page = 1, paginate = false, show_hidden = true } = {}) {
        ...
        return await this.check(async () => {
            ...
            if (paginate) {
                if (!show_hidden)
                    list = list.filter((file) => !/^\./.exec(file));
                total = list.length;
    ...
    async $listfs(dir = '/', { offset = 0, limit = 50, page = 1, paginate = false, show_hidden = true } = {}) {
    ...
    async $removedir(...dirs) {
        return await this.check(async () => {
            ...
            for (let dir of dirs) {
                ...
                if (await this.promises.exists(dir))
                    await this.promises.rmdir(dir);
    ...
    async $__copy__(src, dest) {
        ...
        return await this.check(async () => {
            ...
            const isFile = (src.isdrive
                ...
                : fs.statSync(join(config.fs, src.path))).isFile();
            ...
            if (isFile) {
                ...
                if (!dest.isdrive)
                    dest.path = join(config.fs, dest.path);
                if (!src.isdrive)
                    src.path = join(config.fs, src.path);
