...
async function start() {
    ...
    process.on('SIGINT', async () => {
        ...
        // 	console.log('doen');
        // });
        for (const cleanup of api.cleanups)
            await cleanup();
    ...
    process.on('uncaughtExceptionMonitor', async (err, origin) => {
        ...
        emitter.log(colors.cyan('cleaning up ...'));
        for (let cleanup of api.cleanups)
            await cleanup();
        process.exit();
    });
    ...
    channelList.on('new_channel', async (channel) => {
        ...
        //@ts-ignore
        const isMpvInstalled = (await execChildProcess('apt list --installed mpv')).includes('installed');
        emitter.log('isMpvInstalled', isMpvInstalled);
        api.setIsMpvInstalled(isMpvInstalled);
    });
    // start websocket server
    acceptor.start();
    emitter.log(colors.green(`Connectome → Running websocket connections acceptor on port ${port} ...`));
    //@ts-ignore
